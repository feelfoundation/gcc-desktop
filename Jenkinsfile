...
pipeline {
	...
	parameters {
		...
		string(name: 'FEEL_CORE_VERSION', defaultValue: 'release/3.0.0-beta.1', description: 'Use feel-core branch.', )
		string(name: 'FEEL_CORE_IMAGE_VERSION', defaultValue: '3.0.0-beta.1-a7842d112d5136d9462501763c4cb2895096e900', description: 'Use feel-core docker image.', )
	}
	stages {
		...
		stage('Build') {
			steps {
				parallel (
					...
					"build": {
						withCredentials([string(credentialsId: 'github-feel-token', variable: 'GH_TOKEN')]) {
							nvm(getNodejsVersion()) {
								sh '''
								cp -R /home/feel/fonts/basier-circle src/assets/fonts
								cp -R /home/feel/fonts/gilroy src/assets/fonts
						...
						archiveArtifacts allowEmptyArchive: true, artifacts: 'dist/feel-linux-*'
						...
						ansiColor('xterm') {
							nvm(getNodejsVersion()) {
								...
								script {
									...
									try {
										withCredentials([string(credentialsId: 'feel-hub-coveralls-token', variable: 'COVERALLS_REPO_TOKEN')]) {
					...
					"cypress": {
						dir('feel') {
							checkout([$class: 'GitSCM',
							          branches: [[name: "${params.FEEL_CORE_VERSION}" ]],
								  userRemoteConfigs: [[url: 'https://github.com/feelfoundation/feel-core']]])
						}
						withCredentials([string(credentialsId: 'feel-hub-testnet-passphrase', variable: 'TESTNET_PASSPHRASE')]) {
						withCredentials([string(credentialsId: 'feel-hub-cypress-record-key', variable: 'CYPRESS_RECORD_KEY')]) {
							ansiColor('xterm') {
								wrap([$class: 'Xvfb', parallelBuild: true, autoDisplayName: true]) {
									nvm(getNodejsVersion()) {
										...
										cp -rf $WORKSPACE/feel/docker/ $WORKSPACE/$BRANCH_NAME/
										cp $WORKSPACE/test/dev_blockchain.db.gz $WORKSPACE/$BRANCH_NAME/dev_blockchain.db.gz
										cd $WORKSPACE/$BRANCH_NAME
										cp .env.development .env
										sed -i -r -e "s/ENV_FEEL_VERSION=.*$/ENV_FEEL_VERSION=$FEEL_CORE_IMAGE_VERSION/" .env
...
EOF
										...
										ENV_FEEL_VERSION="$FEEL_CORE_IMAGE_VERSION" make coldstart
										export CYPRESS_baseUrl=http://127.0.0.1:565$N/#/
										export CYPRESS_coreUrl=http://127.0.0.1:$( docker-compose port feel 4000 |cut -d ":" -f 2 )
										cd -
	...
	post {
		...
		fixed {
			script {
				...
				feelSlackSend('good', "Recovery: build ${build_info} was successful.")
			}
		}
		failure {
			script {
				build_info = getBuildInfo()
				feelSlackSend('danger', "Build ${build_info} failed (<${env.BUILD_URL}/console|console>, <${env.BUILD_URL}/changes|changes>)")
			}
		}
		cleanup {
			ansiColor('xterm') {
