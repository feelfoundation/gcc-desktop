...
const getLedgerAccount = (index = 0) => {
  ...
  ledgerAccount.coinIndex(SupportedCoin.FEEL);
  ledgerAccount.account(index);
  return ledgerAccount;
};

/**
 * checkIfInsideFeelApp - function - Validate if after useing the pin to unblock ledger device
 * the user is inside the GCC App, if not then will show the device as connected but not
 ...
 */
const checkIfInsideFeelApp = async ({
  transporter,
  device,
}) => {
  let transport;
  try {
    ...
    const feelLedger = new DposLedger(transport);
    const ledgerAccount = getLedgerAccount();
    const account = await feelLedger.getPubKey(ledgerAccount);
...
const getPublicKey = async (transporter, { device, data }) => {
  ...
  try {
    ...
    const { publicKey: res } = await feelLedger.getPubKey(ledgerAccount, data.showOnDevice);
    transport.close();
    return res;
  } catch (error) {
    if (transport) transport.close();
...
const signTransaction = async (transporter, { device, data }) => {
  ...
  try {
    ...
    const feelLedger = new DposLedger(transport);
    const ledgerAccount = getLedgerAccount(data.index);
    const signature = await feelLedger.signTX(ledgerAccount, getTransactionBytes(data.tx), false);
    transport.close();
