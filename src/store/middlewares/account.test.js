...
describe('Account middleware', () => {
  ...
  const feelAPIClientMock = 'DUMMY_FEEL_API_CLIENT';
  ...
  beforeEach(() => {
    ...
    state = {
      ...
      delegate: {},
      settings: { token: { active: 'GCC' }, statistics: false },
    };
    ...
    jest.spyOn(networkActions, 'networkSet').mockImplementation(() => feelAPIClientMock);
  ...
  it('should show Notification on incoming transaction', () => {
    ...
    expect(windowNotificationSpy).nthCalledWith(
      ...
      {
        body:
        'Your account just received 10 GCC with message Message',
      },
    );
  });
  ...
  it(`should dispatch ${actionTypes.networkSet} action on ${actionTypes.storeCreated} if autologin data found in localStorage`, () => {
    getAutoLogInDataMock.mockImplementation(() => ({
      ...
      [settings.keys.feelCoreUrl]: networks.testnet.nodes[0],
    }));
  ...
  it.skip(`should do nothing on ${actionTypes.storeCreated} if autologin data NOT found in localStorage`, () => {
    middleware(store)(next)(storeCreatedAction);
    expect(store.dispatch).not.toHaveBeenCalledTimes(feelAPIClientMock);
  });
