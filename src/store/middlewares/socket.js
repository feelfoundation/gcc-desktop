...
// only once per minute and not every 10 seconds as we do for GCC
...
const socketSetup = (store) => {
  ...
  const feelAPIClient = getAPIClient(state.settings.token.active, state.network);
  connection = io.connect(feelAPIClient.currentNode);
