...
const getErrorMessage = (errorCode, address) => {
  ...
  switch (errorCode) {
    case 'EUNAVAILABLE':
      ...
      break;
    case 'EPARSE':
      message += i18next.t(' Make sure that you are using the latest version of Feel.');
      break;
    default: break;
  }
  return message;
...
const offlineMiddleware = store => next => (action) => {
  ...
  switch (action.type) {
    case actionsType.feelAPIClientUpdate:
      if (action.data.online === false && network.status.online === true) {
        const address = state.network.networks[settings.token.active || tokenMap.GCC.key].nodeUrl;
