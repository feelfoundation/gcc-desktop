...
import { getAccount } from './api/gcc/account';
import {
...
const signSendTransaction = async (account, data) => {
  const { transfer, utils } = Feel.transaction;
  const transactionObject = {
    ...transfer(data),
    senderPublicKey: account.info.GCC ? account.info.GCC.publicKey : null,
  };
...
const signVoteTransaction = async (
  ...
  const { castVotes, utils } = Feel.transaction;
