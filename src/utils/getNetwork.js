...
export const getNetworkIdentifier = (network) => {
  ...
  const selectedNetwork = (Feel.constants.MAINNET_NETHASH === network.networks.GCC.nethash
    && networks.mainnet)
    || (Feel.constants.TESTNET_NETHASH === network.networks.GCC.nethash && networks.testnet)
    || getNetwork(networkName);
  return !selectedNetwork.custom
    ? selectedNetwork.name.toLowerCase()
    : network.networks.GCC.nethash;
};
...
export const getNetworkNameBasedOnNethash = (network, token = 'GCC') => {
  let activeNetwork = network.name;
  if (network.name === networks.customNode.name && token !== 'BTC') {
    activeNetwork = network.networks[token].nethash === Feel.constants.TESTNET_NETHASH
