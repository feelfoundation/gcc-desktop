...
const testTx = {
  ...
  network: {
    networks: {
      GCC: { networkIdentifier: TESTNET_NETHASH },
    },
  },
};
...
describe('Utils: Transactions API', () => {
  ...
  describe('create', () => {
    ...
    it('should fail for create a transaction and return a promise', async () => {
      Feel.transaction.transfer = jest.fn();
      Feel.transaction.transfer.mockImplementation(() => {
      ...
      Feel.transaction.transfer.mockRestore();
    });
  });
  ...
  describe('getTransactionFee', () => {
    ...
    it('returns transaction type hard cap value', async () => {
      ...
      expect(fees.value).toBeDefined();
      expect(fees.value).toEqual(parseFloat(Number(fromRawGCC(hardCap)).toFixed(8)));
    });
  });
