...
export const findTransactionSizeInBytes = ({
  ...
  const transactionNonce = Feel.cryptography.intToBuffer(
    Number(transaction.nonce),
    byteSizes.nonce,
  );
  const transactionSenderPublicKey = Feel.cryptography.hexToBuffer(transaction.senderPublicKey);
  const txAsset = {
  ...
  const feeBytes = Feel.cryptography.intToBuffer(String(byteSizes.fee), byteSizes.fee);
