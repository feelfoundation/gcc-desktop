...
const Overview = ({ t, txStats }) => {
  ...
  const { txCountList, txVolumeList, txDateList } = txStats.data.timeline.reduce((acc, item) => ({
    ...
    txVolumeList: [...acc.txVolumeList, fromRawGCC(item.volume)],
  }), {
    txCountList: [],
    txDateList: [],
    txVolumeList: [],
  ...
  return (
    <Box className={styles.wrapper}>
      ...
      <BoxContent className={styles.content}>
        ...
        <div className={`${styles.column} ${styles.pie}`}>
          <h2 className={styles.title}>{t('Amount per transaction (GCC)')}</h2>
          <div className={`${styles.graph} showOnLargeViewPort`}>
        ...
        <div className={`${styles.column} ${styles.bar}`}>
          <div className={styles.top}>
            <h2 className={styles.title}>{t('Transactions number / volume (GCC)')}</h2>
            <aside className={styles.legends}>
              <h5 className={`${styles.legend} ${styles.volume}`}>
                <span>{t('Volume')}</span>
                <Tooltip className={styles.tooltip} position="left">
                  <p>{t('The aggregated GCC volume transferred on the given time period.')}</p>
                </Tooltip>
              </h5>
...
export default compose(
  withData(
    {
      txStats: {
        apiUtil: feelService.getTxStats,
        defaultData: {
          distributionByType: {},
