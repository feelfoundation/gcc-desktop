...
const BlockDetails = ({
  ...
  const fields = Object.entries({
    id: {
      ...
      value: (
        <>
          ...
          <CopyToClipboard className="hideOnLargeViewPort" value={blockDetails.data.id ? blockDetails.data.id.replace(regex.gccAddressTrunk, '$1...$3') : ''} />
        </>
      ),
    },
    height: {
    ...
    reward: {
      label: t('Reward'),
      value: <FeelAmount val={blockDetails.data.reward} token={token} />,
    },
    totalFee: {
      label: t('Total fee'),
      value: <FeelAmount val={blockDetails.data.totalFee} token={token} />,
    },
    totalForged: {
      label: t('Total forged'),
      value: <FeelAmount val={blockDetails.data.totalForged} token={token} />,
    },
    totalAmount: {
      label: t('Total amount'),
      value: <FeelAmount val={blockDetails.data.totalAmount} token={token} />,
    },
    date: {
      label: t('Date'),
      value: <DateTimeFromTimestamp
        time={blockDetails.data.timestamp * 1000}
        token={tokenMap.GCC.key} // btc is closed
      />,
    },
    generator: {
