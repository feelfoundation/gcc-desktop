        ...
        <BoxHeader>
          <div>
            ...
            <Tooltip position="right">
              <p>{t('The current list only reflects the peers connected to the Feel Service node.')}</p>
            </Tooltip>
          </div>
        </BoxHeader>
...
export default compose(
  withData({
    networkStatistics: {
      apiUtil: feelService.getNetworkStatistics,
      defaultData: {},
      autoload: true,
      transformResponse: response => response.data,
    },
    peers: {
      apiUtil: feelService.getConnectedPeers,
      defaultData: [],
      autoload: true,
      transformResponse: response => response.data,
    },
