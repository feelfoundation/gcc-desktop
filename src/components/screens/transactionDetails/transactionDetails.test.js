...
describe.skip('Single Transaction Component', () => {
  ...
  const voteTransaction = {
    data: {
      ...
      senderId: accounts.genesis.address,
      recipientId: accounts.delegate.address,
      timestamp: Feel.transaction.utils.getTimeFromBlockchainEpoch() - 100,
      asset: {
        votes: [
  ...
  describe('Transfer transactions', () => {
    ...
    it('Should render transfer transaction with message (GCC)', () => {
      const wrapper = mountWithRouter(
        TransactionDetails,
        { ...props, transaction },
        { pathname: '/explorer/transactions', id: transaction.id },
    ...
    it('Should show the delegate name if the sender is a Feel delegate', () => {
      const delegateTx = {
        data: {
          type: 3,
          senderId: accounts.genesis.address,
