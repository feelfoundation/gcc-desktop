...
class RequestGCC extends React.Component {
  constructor(props) {
    super();

    this.state = {
      shareLink: `feel://wallet/send?recipient=${props.address}`,
      fields: {
        amount: {
          error: false,
          value: '',
  ...
  updateShareLink(newField) {
    ...
    return Object.keys(fields).reduce((link, fieldName) => {
      ...
      return field.value !== ''
        ...
        : link;
    }, `feel://wallet/send?recipient=${address}`);
  }
  ...
  render() {
    ...
    return (
      <RequestWrapper copyLabel={t('Copy link')} copyValue={shareLink} t={t} title={t('Request GCC')}>
        <span className={`${styles.label}`}>
          {t('Use the sharing link to easily request any amount of GCC from Feel users.')}
        </span>
