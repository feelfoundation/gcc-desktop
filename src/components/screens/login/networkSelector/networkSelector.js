...
class NetworkSelector extends React.Component {
  ...
  constructor(props) {
    ...
    if (!loginNetwork) {
      loginNetwork = feelCoreUrl ? networks.customNode : networks.default;
      address = feelCoreUrl || props.address;
    }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (network.name !== prevProps.network.name) {
      this.setState({ activeNetwork: network.networks.GCC.code });
    }
    if (network.name !== prevProps.network.name) {
      this.checkNodeStatus(false);
    }
  ...
  async checkNodeStatus(showErrorToaster = true) {
    const {
      feelAPIClient, network,
    } = this.props;

    if (feelAPIClient) {
      this.setState({
        network: network.networks.GCC.code,
        activeNetwork: network.networks.GCC.code,
      });
      const [error] = await to(feelAPIClient.node.getConstants());
  ...
  validateCorrectNode(network, address, nextPath) {
    ...
    if (network === networks.customNode.code) {
      const feelAPIClient = new Feel.APIClient([nodeURL], {});
      feelAPIClient.node.getConstants()
        .then((res) => {
          if (res.data) {
