...
class AccountVisualWithAddress extends React.Component {
  ...
  render() {
    ...
    const txType = transactionTypes.getByCode(transactionType || 0);
      ...
      <div className={`${styles.address}`}>
        {transactionType !== sendCode && transactionSubject === 'recipientId' ? (
          ...
          <React.Fragment>
            ...
            <span className={`${styles.addressValue} hideOnLargeViewPort`}>{transformedAddress.replace(regex.gccAddressTrunk, '$1...$3')}</span>
          </React.Fragment>
        )}
      </div>
    );
