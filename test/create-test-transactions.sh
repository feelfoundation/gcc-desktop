...
feel config:set api.nodes http://localhost:4500
...
function transfer(){
	feel transaction:broadcast $(feel transaction:create -t 8 $1 0.1 $2 $3 --data=$4 --passphrase="$PASSPHRASE" --networkIdentifier=$NETWORKIDENTIFIER)
}
...
# docker exec -t docker_db_1 pg_dump -U feel feel > ./dev_blockchain.db
